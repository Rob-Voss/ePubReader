      ...
      switch (node.nodeType) {
        ...
        function buildContinuation(i, nextContinuation) {
          if (i < childNodesLength) {
            ...
            descend(childNodes[i], function () {
              buildContinuation(i + 1, nextContinuation);
            });
          } else {
            // No more children to traverse; continue on.
            handler.endElement(node, function () {
              nextContinuation();
            });
          }
        }

          handler.startElement(node, function () {
            buildContinuation(0, continuation);
          });

          break;

        case 3:
          handler.textNode(node, function () {
            continuation();
          });
          break;
